...
from p2p_utils import KademliaP2P, SocketioP2PClient
...
class QuartSIO:
    """Class for instantizing both Quart and SocketIO servers in one object"""
    ...
    def __init__(self) -> None:
        ...
        self.background_tasks = self._quart_app.background_tasks
        self.add_background_task = self._quart_app.add_background_task
...
wghost = WghostUtils()
p2p = KademliaP2P()
sio_client = SocketioP2PClient()


def api_required(func):
    """Wrapper function for testing API credentials"""
...
async def getBlockHash(blockIndex):
    """Returns a block hash for a given block index."""
...
async def getTx(txid, standalone=False):
    """Response for a GET request. Returns raw tx for given txid"""
    ...
    if not isCache:
        if "confirmations" in tx and tx["confirmations"] >= 100:
            ...
            await lvldb.put(
                bytes(txid, "utf-8"), json.dumps(tx, indent=2).encode("utf-8")
            )
...
async def getTxHistory(addrs):
    """GET request. Retruns the transaction history for the given addresses.
    Optional arguments to and from"""
...
async def getTxHistoryPost():
    """POST request. Retruns the transaction history for the given addresses.
    Optional arguments to and from"""
...
async def getUtxo(addr):
    """GET Request. Returns UTXO set for given address."""
...
async def getUtxoMultiGet():
    """POST Request. Returns UTXO set for given addresses."""
    reqJson = json.loads(await request.get_data())
...
async def getUTXOs(addr):
    """Fetches UTXO set for given address from Ghsot Core"""
        ...
        for memUTXO in mempool.copy():
            if "prevtxid" in memUTXO:
                for unspentTx in utxo.copy():
                    if (
                        unspentTx["txid"] == memUTXO["prevtxid"]
                        and unspentTx["outputIndex"] == memUTXO["prevout"]
                    ):
...
async def getInputDetails(inputs, txid):
    """Takes the inputs for a given tx and the TXID and returns the inputs with the addresses and values"""
    ...
    for vin in inputs:
        ...
        if "type" in vin and vin["type"] in ["anon", "blind"]:
            ...
            txType = (
                inTX["vout"][vin["vout"]]["type"]
                if "type" in inTX["vout"][vin["vout"]]
                else "standard"
            )
...
async def getAddrHist(txids, fromIdx, toIdx):
    """Asynchronously retrieves the tx history in the given range."""
    txHistory = []
    txids.reverse()
    ...
    resp = {"totalItems": len(txids), "from": fromIdx, "to": toIdx, "items": results}

    return resp


async def processTxHistoryItem(txid, currHeight):
    """Processes a tx history item async"""
    txHistory = []
    tx = await lvldb.get(bytes(txid, "utf-8"))
    if tx:
        tx = json.loads(tx)
...
def removeBlank(addrLst):
    """Strips blank strings from a list"""
    if addrLst[0] == "":
        addrLst = addrLst[1:]
...
def convertFromSat(value):
    """Converts a int to a float or 8 digits"""
...
async def on_join(sid, data):
    ...
    await app.emit(
        "server_response", f"{username} has entered the room. {room}", room=room, to=sid
    )
...
async def startup():
    ...
    app.add_background_task(vinDetailCleanup)

    # print(f"""your value: {await p2p.get("nice")}""")

    # await p2p.set("thing", "cool value")

    # print(f"""your value: {await p2p.get("thing")}""")

    # print(p2p.server.bootstrappable_neighbors())

    # # print(p2p.server.storage.iter_older_than(0))
    # print(p2p.server.storage.iter_newer_than(1000000))

    # print(p2p.server.storage.data)

    # print(p2p.server.protocol.router.find_neighbors(p2p.server.node))

    # print(p2p.server.node.id.hex())

    app.add_background_task(sio_client.add_connection, "127.0.0.1", 52556)
...
async def shutdown():
    print("here")
    await sio_client.disconnect_all()
        ...
        for item in vinDetail:
            ...
            if "confirmations" in tx and tx["confirmations"] >= 100:
                if not await lvldb.get(bytes(item[0], "utf-8")):
                    ...
                    tx["isCoinStake"] = isCoinStake
                    if isCoinStake:
                        rewardDetails = await callrpc(
                            PORT, "getblockreward", [int(tx["height"])]
                        )
                        ...
                        if (
                            "gvrreward" in rewardDetails
                            and rewardDetails["blockreward"] > 0
                        ):
                            tx["isAGVR"] = True
                            tx["rewardAGVR"] = float(rewardDetails["gvrreward"])
                            tx["rewardAGVRSat"] = convertToSat(
                                rewardDetails["gvrreward"]
                            )
                        else:
                            tx["isAGVR"] = False
                    ...
                    await lvldb.put(
                        bytes(item[0], "utf-8"),
                        json.dumps(tx, indent=2).encode("utf-8"),
                    )
